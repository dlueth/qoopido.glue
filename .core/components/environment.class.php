...
final class Environment extends \Glue\Abstracts\Base\Singleton {
	...
	final public function onPreRender() {
		\Glue\Factory::getInstance()->get('\Glue\Adapter\View')->register('environment', $this->registry->get());
	}
	...
	final protected function __initialize() {
		try {
			...
			$url      = \Glue\Components\Url::getInstance();
			$data     = array();
			...
			// set secondary environment variables
				...
				$data['compression']          = ($data['compression'] !== false && extension_loaded('zlib') === true && preg_match('/gzip|deflate/', \Glue\Components\Client::getInstance()->get('accept.encoding'))) ? $data['compression'] : false;
