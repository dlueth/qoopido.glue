...
final class Url extends \Glue\Abstracts\Base\Singleton {
	...
	final protected function __initialize() {
		try {
			...
			$configuration = \Glue\Components\Configuration::getInstance();
			...
			$path['local'] = $path['global'] . '/' . ($this->registry->get('switches.site') ?: $configuration->get('\Glue\Components\Environment\defaults\site'));
