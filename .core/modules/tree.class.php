 ...
 * Tree module
 *
 * @event glue.modules.tree.load.pre(array $files) > load()
 * @event glue.modules.tree.load.post(array $files, array $data) > load()
 *
...
class Tree extends \Glue\Abstracts\Base {
	...
	public static function __once() {
		try {
			...
			self::$node = '/' . \Glue\Components\Environment::getInstance()->get('node');
