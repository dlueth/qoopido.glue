 ...
 *
 * @event glue.modules.database.connect.pre() > connect()
 * @event glue.modules.database.connect.post() > connect()
 *
...
class Database extends \Glue\Abstracts\Base {
	...
	protected function __initialize($configuration = array()) {
		try {
			$this->configuration = \Glue\Helper\General::merge((array) \Glue\Components\Configuration::getInstance()->get(__CLASS__), $configuration);
