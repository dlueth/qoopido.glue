 ...
 * Language module
 *
 * @event glue.modules.language.load.pre(array $files) > load()
 * @event glue.modules.language.load.post(array $files, array $data) > load()
 * @event glue.modules.language.apply.pre(array $data) > apply()
 * @event glue.modules.language.apply.post(array $data) > apply()
 *
		...
		try {
			...
			// alter environment
				if(isset($registry['@attributes']['code'])) {
					\Glue\Components\Environment::getInstance()->set('language', $registry['@attributes']['code']);
				}

				if(isset($registry['@attributes']['characterset'])) {
					\Glue\Components\Environment::getInstance()->set('characterset', $registry['@attributes']['characterset']);
				}
